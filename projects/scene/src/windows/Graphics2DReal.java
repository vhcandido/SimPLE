...
public class Graphics2DReal {
    ...
    public void drawLineR(Point2D p1, Point2D p2) {
        drawLineR(p1.getX(), p1.getY(), p2.getX(), p2.getY());
    }
    ...
    public void paintPolygon(Point2D[] points, Color draw) {
    ...
    public void paintPolygon(Point2D[] points, String name, double size, Color fill, Color draw) {
        paintPolygon(points, fill, draw);
        setColor(draw);
        double mx = averageX(points);
        double my = averageY(points);
        drawStringR(name, new Font(Font.SANS_SERIF, Font.BOLD, (int)(size*2)+1), mx, my);
    }
...
//    }
    ...
    public void drawStringR(double x, double y, double size, String format, Object ...args) {
        Font font = new Font(Font.SANS_SERIF, Font.BOLD, (int)(size*2)+1);
        drawStringR(format, font, x, y, args);
    }
    ...
    public void draw(Rectangle2D rect, Color color) {
        g2.setColor(color);
        g2.drawRect(unit(rect.getX()), unit(rect.getY()), unit(rect.getWidth()), unit(rect.getHeight()));
    }
    public void draw(Shape draw_region) {
        Rectangle r = draw_region.getBounds();
        double cx = r.x+r.width/2.0;
        double cy = r.y+r.height/2.0;
        BasicStroke save = null;
        if(g2.getStroke() instanceof BasicStroke){
            save = (BasicStroke) g2.getStroke();
            g2.setStroke(new BasicStroke(save.getLineWidth()/UNIT));
        }
